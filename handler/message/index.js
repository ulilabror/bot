    ...
    try {
        ...
        case 'facebook':
            ...
            downloader.facebook(url).then(async (videoMeta) => {
                ...
                const url = videoMeta.response.links[0].url
                const shorts = []
                for (let i = 0; i < links.length; i++) {
                    const shortener = links[i].url
                ...
                await client.sendFileFromUrl(from, url, 'videos.mp4', caption, null, null, true)
                    .then((serialized) => console.log(`Sukses Mengirim File dengan id: ${serialized} diproses selama ${processTime(t, moment())}`))
                    .catch((err) => console.error(err))
            })
            ...
            case 'ytmp4':
            if (args.length !== 1) return client.reply(from, 'Maaf, format pesan salah silahkan periksa menu. [Wrong Format]', id)
            if (!isUrl(url) && !url.includes('youtu.be')) return client.reply(from, 'Maaf, url yang kamu kirim tidak valid. [Invalid Link]', id)
            await client.reply(from, '_Scraping Metadata..._ \n\nTerimakasih telah menggunakan bot ini.', id)
            downloader.ytmp4(url).then(async (videoMeta) => {
                const title = videoMeta.response.title
                const thumbnail = videoMeta.response.thumbnail
                const links = videoMeta.response.links
                const url = videoMeta.response.links[0].url
